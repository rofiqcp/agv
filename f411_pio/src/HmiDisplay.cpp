...
void HmiDisplay::command(uint8_t value) {
  setSpiPrescaler(SPI_BAUDRATEPRESCALER_8);
...
void HmiDisplay::drawCircle(int32_t x0,int32_t y0,int32_t r,uint16_t color){
  if(r<0)return;int32_t f=1-r,ddx=1,ddy=-2*r,x=0,y=r;drawPixel(x0,y0+r,color);drawPixel(x0,y0-r,color);drawPixel(x0+r,y0,color);drawPixel(x0-r,y0,color);
...
void HmiDisplay::drawRoundRect(int32_t x,int32_t y,int32_t w,int32_t h,int32_t r,uint16_t color){
  if(w<=0||h<=0)return;r=std::max<int32_t>(0,std::min<int32_t>(r,std::min(w,h)/2));drawFastHLine(x+r,y,w-2*r,color);drawFastHLine(x+r,y+h-1,w-2*r,color);drawFastVLine(x,y+r,h-2*r,color);drawFastVLine(x+w-1,y+r,h-2*r,color);
...
void HmiDisplay::fillRoundRect(int32_t x,int32_t y,int32_t w,int32_t h,int32_t r,uint16_t color){
  if(w<=0||h<=0)return;r=std::max<int32_t>(0,std::min<int32_t>(r,std::min(w,h)/2));fillRect(x+r,y,w-2*r,h,color);
...
void HmiDisplay::pushImage(int32_t x,int32_t y,int32_t w,int32_t h,const uint16_t *pixels){
  if(pixels==nullptr||w<=0||h<=0||x<0||y<0||x+w>width_||y+h>height_)return;setWindow(x,y,w,h);uint8_t block[256];const uint32_t total=static_cast<uint32_t>(w*h);uint32_t pos=0U;setSpiPrescaler(SPI_BAUDRATEPRESCALER_8);TouchCs(true);TftCs(false);TftDc(true);
...
int16_t HmiDisplay::drawString(const char *text,int32_t x,int32_t y){
  if(text==nullptr)return 0;int32_t minx,miny,maxx,maxy,advance;textBounds(text,minx,miny,maxx,maxy,advance);const int32_t cw=std::max<int32_t>(0,maxx-minx+1),ch=std::max<int32_t>(1,maxy-miny+1),lw=std::max<int32_t>(advance,cw);int32_t left=x,top=y;
