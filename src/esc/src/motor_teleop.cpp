...
public:
  MotorTeleop() : Node("motor_teleop") {
    ...
    // active_source adalah state, bukan stream sesaat. Samakan durability dengan
    // cmd_vel_router/web subscriber agar subscriber baru langsung menerima state
    // terakhir dan Fast DDS tidak menghasilkan DURABILITY_QOS_POLICY warning.
    source_pub_ = create_publisher<std_msgs::msg::String>(
        "/teleop/active_source",
