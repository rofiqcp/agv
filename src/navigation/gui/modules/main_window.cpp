...
class MainWindow:public QMainWindow{
  ...
  void syncVehicleAuthority(){
    ...
    // velocity_smoother is shared by autonomous yaw-rate commands and manual teleop.
    // Teleop intentionally encodes normalized steering in angular.z: +/-yaw_max_deg_s
    // must survive this transport so full stick can reach the physical +/-30 deg clamp.
    // Autonomous yaw remains independently capped by MPPI/navigation_core/trajectory safety.
    double teleopYawEncodingRps=80.0*kPi/180.0;
    if(auto teleop=stores_.value("teleop")){
      const double deg=number(teleop->get("/**.ros__parameters.yaw_max_deg_s",80.0),80.0);
      teleopYawEncodingRps=std::max(0.01,deg*kPi/180.0);
    }
    const double smootherYaw=std::max(navYaw,teleopYawEncodingRps);
    set("nav2","velocity_smoother.ros__parameters.max_velocity",QVariantList{
      navV,0.0,smootherYaw
    });
