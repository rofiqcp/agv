...
class ExperimentParameterPanel : public QWidget {
  ...
  signals:
  // Emitted when an editable field changes. YAML commits expose file/path too.
  void parameterEdited(const QString &key, const QVariant &value);
  void yamlParameterCommitted(const QString &fileKey, const QString &path, const QVariant &value);
  private:
