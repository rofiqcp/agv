...
class NavigationTuningPage:public QWidget{
  ...
  void applyProfile(){
    ...
    // Shared smoother must pass the manual steering encoding unchanged.
    double teleopYawEncodingRps=80.0*kPi/180.0;
    if(auto teleop=s_.value("teleop")){
    ...
    const double smootherYaw=std::max(wz,teleopYawEncodingRps);
