...
inline QVector<ExperimentSpec> buildExperimentCatalog(const QString &subsystem) {
  ...
  if (subsystem == QStringLiteral("navigation")) {
    ...
    // IMPORTANT: ESC command target and ESC feedback/odom actual rate are different quantities.
    add("navigation",QStringLiteral("N0"),QStringLiteral("N0 System, Topic & Timing"),"N0.1",
      ...
      {"Actual output rate terhadap Time [s] — target konfigurasi ditampilkan terpisah","Message age aktual terhadap Time [s]","Inter-arrival time sumber aktual terhadap Time [s]"},
      {{"Variasi","Mean GNSS Actual [Hz]","Mean IMU Actual [Hz]","Mean ESC Feedback /odom Actual [Hz]","Mean EKF Local Actual [Hz]","Mean EKF Global Actual [Hz]","Max Age GNSS [s]","Max Age IMU [s]","Max Age ESC Feedback [s]"}},
      {{"GNSS Actual","derived.rate_gnss_hz"},{"IMU Actual","derived.rate_imu_hz"},{"ESC Feedback /odom Actual","derived.rate_esc_hz"},{"EKF Local Actual","derived.rate_ekf_local_hz"},{"EKF Global Actual","derived.rate_ekf_global_hz"},
       {"Age GNSS","derived.age_gnss_s"},{"Age IMU","derived.age_imu_s"},{"Age ESC Feedback","derived.age_esc_s"},{"Age EKF Local","derived.age_ekf_local_s"},{"Age EKF Global","derived.age_ekf_global_s"},
       {"dt GNSS","derived.dt_gnss_s"},{"dt IMU","derived.dt_imu_s"},{"dt ESC Feedback","derived.dt_esc_s"}},
      P({RO("gnss_rate_cfg","GNSS production target — NEO3 Pro source [Hz]","hmi","stmf4_hmi_bridge.ros__parameters.neo3pro_gnss_rate_hz","TARGET 10 Hz"),
         RO("gnss_rate_gate_cfg","GNSS minimum usable rate [Hz]","hmi","stmf4_hmi_bridge.ros__parameters.neo3pro_min_usable_rate_hz","FAIL-CLOSED GATE — navigation/fusion require >7 Hz"),
         RO("imu_rate_cfg","IMU target publish rate [Hz]","imu","data_imu_node.ros__parameters.publish_rate_hz","CONFIG TARGET — dibandingkan dengan IMU Actual"),
         RO("esc_rate_cfg","ESC command-loop target rate [Hz]","esc","esc_ackermann.ros__parameters.command_rate_hz","CONFIG TARGET ONLY — jangan dibandingkan 1:1 dengan /esc/odom feedback"),
         RO("local_ekf_rate_cfg","EKF Local target frequency [Hz]","ekf","ekf_filter_node_odom.ros__parameters.frequency","CONFIG TARGET — dibandingkan dengan EKF Local Actual"),
         RO("global_ekf_rate_cfg","EKF Global target frequency [Hz]","ekf","ekf_filter_node_map.ros__parameters.frequency","CONFIG TARGET — dibandingkan dengan EKF Global Actual")}),
      {{"time_series",{"GNSS Actual","IMU Actual","ESC Feedback /odom Actual","EKF Local Actual","EKF Global Actual"},{},{},"Time [s]","Actual output rate [Hz]"},
       {"time_series",{"Age GNSS","Age IMU","Age ESC Feedback","Age EKF Local","Age EKF Global"},{},{},"Time [s]","Message age [s]"},
       {"time_series",{"dt GNSS","dt IMU","dt ESC Feedback"},{},{},"Time [s]","Inter-arrival Δt [s]"}});
    ...
    add("navigation",QStringLiteral("N2"),QStringLiteral("N2 Drive Odometry Calibration"),"N2.1",
      QStringLiteral("N2.1 eRPM, Velocity dan Distance Scale"),
      {"eRPM ESC selama trial","Velocity ESC raw/calibrated, GNSS, dan IMU diagnostic","Perpindahan GNSS X-Y","eRPM/(m/s) candidate per trial"},
      {{"Pengujian","eRPM set","Mean eRPM ESC","Mean V ESC raw","Mean V GNSS","Mean V ESC calibrated","Mean V IMU","Delta X","Delta Y","Distance GNSS","eRPM/(m/s) candidate","Mean hAcc","Samples"},
       {"Parameter","Nilai final","Status / sumber"}},
      {{"eRPM ESC","vesc_right_values.rpm"},{"V ESC raw","esc_drive_raw"},{"V ESC calibrated","esc_odom.v"},
       {"V GNSS","gnss_vel.speed"},{"V IMU","imu_speed_kalman"},{"GNSS X","gnss_map_odom.x"},{"GNSS Y","gnss_map_odom.y"},
       {"hAcc","gnss_quality.hacc_m"}},
      P({F("test_erpm","eRPM ESC RIGHT untuk trial","float",{}, {},"200","VARIASI TRIAL"),
         F("trial_duration","Durasi rekomendasi per trial (s)","float",{}, {},"15","VARIASI TRIAL"),
         RO("erpm_per_mps","Drive eRPM per m/s — runtime","esc","esc_ackermann.ros__parameters.drive_erpm_per_mps","CALIBRATION RESULT"),
         RO("vehicle_erpm_per_mps","Drive eRPM per m/s — vehicle authority","vehicle","vehicle.ros__parameters.drive_erpm_per_mps","PERSISTENT RESULT"),
         ...
         RO("odom_saved_at","Drive calibration saved at","vehicle","vehicle.ros__parameters.drive_odometry_calibration_saved_at","CERTIFICATION"),
      ...
      {{"time_series",{"eRPM ESC"},{},{},"Time [s]","VESC eRPM"},
    ...
    add("navigation",QStringLiteral("N3"),QStringLiteral("N3 Steering & Ackermann Calibration"),"N3.1",
      QStringLiteral("N3.1 Straight Steering Zero dengan Gyro, RM3100 dan GNSS"),
      {"Steering target/actual saat lurus","Heading IMU inertial, RM3100, dan GNSS COG","Yaw-rate gyro dan model","Lintasan GNSS X-Y"},
      {{"Pengujian","eRPM set","Mean V GNSS","Steering target","Mean Steering actual","Mean Gyro Z","Delta Yaw IMU","Delta Yaw RM3100","Delta COG GNSS","Delta X","Delta Y","Lateral drift","Center candidate","Samples"}},
      {{"Steer target","esc_steer_target"},{"Steer actual","esc_steer_actual"},{"Gyro Z","imu.gz"},{"Yaw IMU","imu.yaw_rad"},
       {"Yaw RM3100","rm3100_heading.yaw_rad"},{"Yaw GNSS COG","gnss_cog_fusion.yaw_rad"},
       {"V GNSS","gnss_vel.speed"},{"GNSS X","gnss_map_odom.x"},{"GNSS Y","gnss_map_odom.y"}},
      P({F("test_erpm","eRPM ESC RIGHT","float",{}, {},"200","VARIASI SPEED"),
      ...
      {{"time_series",{"Steer target","Steer actual"},{},{},"Time [s]","Steering [rad]"},
       {"time_series",{"Yaw IMU","Yaw RM3100","Yaw GNSS COG"},{},{},"Time [s]","Heading [rad]"},
    ...
    add("navigation",QStringLiteral("N3"),QStringLiteral("N3 Steering & Ackermann Calibration"),"N3.2",
      ...
      {{"Pengujian","eRPM set","Steering set [deg]","Mean Steering actual","Mean V GNSS","Mean V ESC","Mean Gyro Z","Mean Yaw-rate model","Radius GNSS","Radius model","Effective wheelbase candidate","Scale steering candidate","Delta COG","Mean hAcc","Samples"},
      ...
      {{"Steer target","esc_steer_target"},{"Steer actual","esc_steer_actual"},{"V GNSS","gnss_vel.speed"},{"V ESC","esc_odom.v"},
       {"Gyro Z","imu.gz"},{"Yaw-rate model","esc_kinematic_yaw_rate"},{"Yaw IMU","imu.yaw_rad"},
       {"Yaw RM3100","rm3100_heading.yaw_rad"},{"Yaw GNSS COG","gnss_cog_fusion.yaw_rad"},{"GNSS X","gnss_map_odom.x"},{"GNSS Y","gnss_map_odom.y"}},
      P({F("test_erpm","eRPM ESC RIGHT","float",{}, {},"200","VARIASI SPEED"),
    ...
    add("navigation",QStringLiteral("N5"),QStringLiteral("N5 GNSS Qualification"),"N5.1",
      QStringLiteral("N5.1 Source Rate, Quality, DOP, Position Stability dan Timing"),
      ...
      P({F("nav_rate","NEO3 Pro source-rate target (Hz)","int","hmi","stmf4_hmi_bridge.ros__parameters.neo3pro_gnss_rate_hz","10","Receiver Timing — production target 10 Hz"),
         RO("min_usable_rate","GNSS minimum usable rate (Hz)","hmi","stmf4_hmi_bridge.ros__parameters.neo3pro_min_usable_rate_hz","Fail-closed flow gate; threshold 7 Hz; actual must be >7 Hz"),
         F("min_sat","Strict minimum satellites","int","localization","localization_core.ros__parameters.strict_min_satellites","8","Motion Quality Gate"),
         F("max_dop","Strict maximum DOP","float","localization","localization_core.ros__parameters.strict_max_dop","2.0","Motion Quality Gate"),
         F("max_hacc","Strict maximum hAcc (m)","float","localization","localization_core.ros__parameters.strict_max_hacc_m","2.5","Motion Quality Gate"),
         RO("stm32_source","STM32 GNSS publisher enabled","hmi","stmf4_hmi_bridge.ros__parameters.publish_stm32_gnss","Source Authority")}),
    ...
    add("navigation",QStringLiteral("N17"),QStringLiteral("N17 Final Certified Configuration"),"N17.1",
      ...
      P({RO("steering_valid","Steering calibration valid","vehicle","vehicle.ros__parameters.steering_calibration_valid","Certification"),
         RO("drive_erpm_per_mps","Drive eRPM per m/s","vehicle","vehicle.ros__parameters.drive_erpm_per_mps","Certification"),
         RO("gnss_rate","NEO3 Pro source-rate target","hmi","stmf4_hmi_bridge.ros__parameters.neo3pro_gnss_rate_hz","Certification"),
         RO("circle_valid","Circle calibration valid","vehicle","vehicle.ros__parameters.steering_circle_calibration_valid","Certification"),
