...
def generate_launch_description() -> LaunchDescription:
    ...
    configured_pt = str(_yaml_ros_param(camera_params, 'perception', 'pt_model_path', '/home/otomasi/ros/models/yolopv2.pt') or '/home/otomasi/ros/models/yolopv2.pt')
    pt_model_path = os.environ.get('YOLOPV2_PT_PATH', configured_pt)
    cpu_fps_default = float(_yaml_ros_param(
        camera_params, 'perception', 'cpu_inference_fps', 1.0))
    cpu_threads_default = int(_yaml_ros_param(
        camera_params, 'perception', 'cpu_threads', 4))
    ...
    common_perception_parameters = {
        ...
        'nav_cmd_topic': '/cmd_vel_nav_smoothed',
    ...
    smoother = Node(
        ...
        remappings=[('cmd_vel', '/cmd_vel_nav_raw'), ('cmd_vel_smoothed', '/cmd_vel_nav_smoothed')])
    ...
    # Velocity smoother is an independent command-conditioning stage and must be
    # ACTIVE from launch. Previously it shared lifecycle_manager_navigation with
    # autostart=False, so /cmd_vel_nav_smoothed could stay unavailable until
    ...
    return LaunchDescription(args + environment + [
        ...
        LogInfo(msg='[AGV] autonomous stack | velocity smoother + measured steering calibration + safety gates enabled'),
        LogInfo(
        ...
        LogInfo(
            ...
            msg='[AGV] PERCEPTION CPU: LibTorch membaca /home/otomasi/ros/models/yolopv2.pt langsung; ONNX tidak digunakan.'),
        LogInfo(
        ...
        robot_state, gnss, imu, esc_runtime,
        local_ekf, global_ekf, localization_core,
        camera_only, perception_cpu, perception_gpu,
        map_server, lifecycle_map, controller, planner, behavior, smoother, collision, navigator,
        lifecycle_smoother, lifecycle_with_collision, lifecycle_without_collision,
        trajectory_safety, navigation_core, mppi_closed_loop, rviz,
    ])
