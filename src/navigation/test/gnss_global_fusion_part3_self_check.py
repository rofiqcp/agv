...
if float(loc.get('cog_min_forward_speed_mps',0.0)) < 0.35: fail('COG low-speed gate must stay conservative (>=0.35 m/s)')
...
# Local EKF: odom frame continuity — GNSS vx + IMU absolute yaw (unchanged).
if l.get('twist0')!='/gnss/base_velocity_fusion' or enabled(l.get('twist0_config')) != [6]:
    fail('local EKF must fuse independent GNSS vx')
if l.get('imu0')!='/imu/data' or enabled(l.get('imu0_config')) != [5,11]:
    fail('local EKF must fuse IMU absolute yaw + gyro-Z')
if l.get('odom0')!='/esc/odom' or enabled(l.get('odom0_config')) != [6,11]: fail('local EKF optional ESC vx+kinematic vyaw missing')
...
print('PASS GNSS vx/vyaw + IMU yaw fusion contract')
