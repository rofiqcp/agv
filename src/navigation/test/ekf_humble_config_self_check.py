...
def main() -> None:
    ...
    if local.get("imu0") != "/imu/data" or enabled(local.get("imu0_config", [])) != {5, 11}:
    ...
    if global_.get("pose0") != "/gnss/cog_heading_fusion" or enabled(global_.get("pose0_config", [])) != {5}:
        fail("global EKF must fuse GNSS COG absolute yaw")
    if global_.get("imu0") != "/imu/data" or enabled(global_.get("imu0_config", [])) != {11}:
        fail("global EKF must fuse IMU gyro-Z yaw-rate continuity")
    print("PASS EKF sensor ownership: GNSS=vx+vyaw, IMU=yaw")
