...
function expSheetXml(tableIndex){
  const {head,rows,yamlStart}=expTableMatrix(tableIndex),title=`${displayExperimentId(selectedExperiment)} ${displayExperimentTitle(selectedExperiment)}`;
  ...
  const srcInfo=selectedExperiment?.reportMode?` | Source Tuning: ${(selectedExperiment.sourceIds||[]).join(', ')}`:'';
  xml+=`<row r="3">${expCell('A3',`Variation: ${document.getElementById('runVariation')?.value||''} | Condition: ${document.getElementById('runCondition')?.value||''} | 1 row/detik${srcInfo}`,4)}</row><row r="4"></row>`;
  xml+=`<row r="5">${head.map((h,i)=>expCell(`${expColName(i)}5`,h,i>=yamlStart?5:2)).join('')}</row>`;
  rows.forEach((r,ri)=>{const rn=ri+6;xml+=`<row r="${rn}">${r.map((v,ci)=>expCell(`${expColName(ci)}${rn}`,v,ci>=yamlStart?6:3)).join('')}</row>`});
...
function expStylesXml(){return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="3"><font><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Calibri"/></font><font><b/><color rgb="FF173F35"/><sz val="15"/><name val="Calibri"/></font></fonts><fills count="6"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF1F4E78"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFD9EAD3"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF8A4FD1"/><bgColor indexed="64"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF0E5FF"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border/><border><left style="thin"><color rgb="FFD0D7D5"/></left><right style="thin"><color rgb="FFD0D7D5"/></right><top style="thin"><color rgb="FFD0D7D5"/></top><bottom style="thin"><color rgb="FFD0D7D5"/></bottom></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="7"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="2" fillId="3" borderId="0" xfId="0" applyFont="1" applyFill="1"/><xf numFmtId="0" fontId="1" fillId="2" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" xfId="0" applyBorder="1"><alignment vertical="top" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"><alignment vertical="center"/></xf><xf numFmtId="0" fontId="1" fillId="4" borderId="1" xfId="0" applyFont="1" applyFill="1" applyBorder="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="5" borderId="1" xfId="0" applyFill="1" applyBorder="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs></styleSheet>`}
async function downloadLastNavigationExcel(){
  const url=lastTrialArtifacts?.xlsx_download_url;if(!url)return false;
...
async function saveExperimentExcel(){
  ...
  if(!tables.length){if(currentExp==='navigation'&&await downloadLastNavigationExcel())return;return toast('Belum ada data hasil START/STOP untuk dibuat Excel',true)};
...
function saveAllGuiCsv(){
  ...
  for(let i=0;i<(selectedExperiment.tableColumns||[]).length;i++){const csv=expGuiCsv(i);if(!csv)continue;downloadBlob(`${safe}_T${i+1}_${tag}.csv`,'text/csv;charset=utf-8',currentExp==='navigation'?'\ufeff'+csv:csv);saved++}
