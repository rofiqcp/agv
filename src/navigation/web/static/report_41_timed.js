...
const R41_PROFILES={
  ...
  'R4.1.4':{title:'Karakteristik Heading RM3100 & Inersia • Akuisisi 60 Detik',targetHz:50,primary:'imu',channels:['rm3100_heading','r41_inertia_heading','imu','rm3100_mag'],countLabels:{rm3100_heading:'N RM3100 heading',r41_inertia_heading:'N Inersia',imu:'N IMU',rm3100_mag:'N RM3100 field'},detailHint:'Heading RM3100 dan inersia direkam pada rate sumber aktual; gyro IMU ditargetkan 50 Hz. Mean heading dihitung secara circular, bukan mean aritmatika biasa.',metrics:[
    {key:'rm3100_compass',label:'RM3100 compass',unit:'°',channel:'rm3100_heading',angular:true,pick:v=>r41RadToCompassDeg(v?.yaw_rad)},
    ...
    {key:'delta_heading',label:'Δ Inersia−RM3100',unit:'°',channel:'r41_inertia_heading',pick:()=>report41HeadingMetric('derived.r41_heading_delta_deg')},
    ...
    {key:'field_norm',label:'RM3100 field norm',unit:'µT',channel:'rm3100_mag',pick:v=>r41Num(v?.norm_ut)}]}
};
...
function r41HeadingPair(){const n=r41Num(state?.rm3100_heading?.yaw_rad),ih=r41InertiaHeading(),neo=r41RadToCompassDeg(n),inertia=r41RadToCompassDeg(ih.yawRad);return{neo,inertia,inertiaSource:ih.source,inertiaDirect:ih.direct,delta:Number.isFinite(neo)&&Number.isFinite(inertia)?r41WrapDeg180(inertia-neo):NaN}}
function report41HeadingMetric(path){const h=r41HeadingPair();if(path==='derived.r41_heading_rm3100_compass_deg')return h.neo;if(path==='derived.r41_heading_inertia_compass_deg')return h.inertia;if(path==='derived.r41_heading_delta_deg')return h.delta;if(path==='derived.r41_heading_drift_deg'){if(!r41Session||r41Session.id!=='R4.1.4'||!Number.isFinite(h.delta))return NaN;if(!Number.isFinite(r41Session.headingBaselineDeltaDeg))r41Session.headingBaselineDeltaDeg=h.delta;return r41WrapDeg180(h.delta-r41Session.headingBaselineDeltaDeg)}return NaN}
