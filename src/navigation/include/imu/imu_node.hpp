...
private:
  ...
  bool looksLikeWitStream(double probe_sec = 0.6);
  std::vector<int> candidateBaudrates() const;
  ...
  // A seconds-old angle packet must not be re-stamped as a current heading.
  double orientation_publish_timeout_sec_ = 0.50;
  // Field-level freshness is intentionally much tighter than stream-recovery
  // timeout. A live ANGLE packet must never re-stamp an old gyro sample and
  // feed stale angular velocity into robot_localization.
