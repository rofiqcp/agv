...
def fit_heading_only(rows,xkey,ykey,label,unit):
    per={s:[r for r in rows if r['segment']==s and finite(r.get(xkey)) and finite(r.get(ykey))] for s in range(1,STOP_COUNT+1)}
    counts={s:len(per[s]) for s in per}
    if any(counts[s]<MIN_SAMPLES_PER_STOP for s in range(1,STOP_COUNT+1)):
        raise ValueError(f'{label}: sample per stop kurang: {counts}')
    means=[]
    for s in FIT_SEGMENTS:
        q=per[s]; x=float(np.mean([r[xkey] for r in q])); y=float(np.mean([r[ykey] for r in q]))
        means.append((s,x,y,enu_target_rad(s,rows[0]['direction'])))
    alpha=np.array([math.atan2(m[2],m[1]) for m in means]); target=np.array([m[3] for m in means])
    best=None
    for sign in (-1.0,1.0):
        off=cmean([wrap(float(t-sign*a)) for t,a in zip(target,alpha)])
        base=np.array([wrap(sign*float(a)+off) for a in alpha]);err=np.array([wrap(float(t-b)) for t,b in zip(target,base)])
    ...
    _,sign,off,base,err=best;knots=[float(v) for v in base];corr=[float(v) for v in err]
    fit_err=[];closure_err=[];norms=[];per_stop=[]
    model={'bias':[0.0,0.0],'matrix':[1.0,0.0,0.0,1.0],'yaw_sign':float(sign),'yaw_offset_rad':float(off),
           'heading_lut_input_rad':knots,'heading_lut_correction_rad':corr}
    for s in range(1,STOP_COUNT+1):
        hs=[];ns=[];tgt=enu_target_rad(s,rows[0]['direction'])
        for r in per[s]:
            h,n=model_heading(model,r[xkey],r[ykey]);hs.append(h);ns.append(n);norms.append(n)
        ...
        mean_h=cmean(hs);per_stop.append({'segment':s,'target_compass_deg':compass_target_deg(s,rows[0]['direction']),
          'target_enu_deg':math.degrees(tgt),'predicted_enu_deg':math.degrees(mean_h),'mean_error_deg':math.degrees(wrap(mean_h-tgt)),
          'samples':len(hs),'field_norm_mean_ut':float(np.mean(ns))})
    arr=np.asarray(norms,dtype=float);p05=float(np.percentile(arr,5));p95=float(np.percentile(arr,95))
    fr=rms_deg(fit_err);fm=math.degrees(max(abs(e) for e in fit_err));cr=rms_deg(closure_err);cm=math.degrees(max(abs(e) for e in closure_err));cmean_abs=abs(math.degrees(cmean(closure_err)))
    field_ok=p05>=15.0 and p95<=100.0
    valid=fr<FIT_RMS_MAX_DEG and fm<FIT_MAX_ERROR_DEG and cmean_abs<=CLOSURE_MAX_DEG and cm<FIT_MAX_ERROR_DEG and field_ok
    return {'valid':bool(valid),'sensor':label,'unit':unit,'owner':'ap_periph','host_hard_soft_iron':False,
      'sample_count':sum(counts.values()),'fit_sample_count':sum(counts[s] for s in FIT_SEGMENTS),'closure_sample_count':counts[CLOSURE_SEGMENT],
      'yaw_sign':model['yaw_sign'],'yaw_offset_rad':model['yaw_offset_rad'],'heading_lut_input_rad':knots,'heading_lut_correction_rad':corr,
      'field_norm_mean_ut':float(np.mean(arr)),'field_norm_std_ut':float(np.std(arr)),'field_norm_p05_ut':p05,'field_norm_p95_ut':p95,
      'field_norm_gate_pass':bool(field_ok),'per_stop':per_stop,
      'validation':{'fit_rms_error_deg':fr,'fit_max_abs_error_deg':fm,'closure_rms_error_deg':cr,'closure_max_abs_error_deg':cm,
        'closure_mean_error_deg':math.degrees(cmean(closure_err)),'rms_error_deg':fr,'max_abs_error_deg':fm,'pass':bool(valid)}}
...
def cross_sensor(rows,yah,neo):
    ...
    for s in range(1,STOP_COUNT+1):
        ...
        for r in rows:
            ...
            nh,_=model_heading({'bias':neo.get('bias',[0.0,0.0]),'matrix':neo.get('matrix',[1.0,0.0,0.0,1.0]),'yaw_sign':neo['yaw_sign'],'yaw_offset_rad':neo['yaw_offset_rad'],'heading_lut_input_rad':neo['heading_lut_input_rad'],'heading_lut_correction_rad':neo['heading_lut_correction_rad']},r['neo_mag_x_ut'],r['neo_mag_y_ut'])
...
def main():
    ...
    out={'heading_360_calibration':unified,'yahboom_mag_planar_calibration':legacy,
      'neo3_planar_candidate':{'valid':neo['valid'],'stage1_only':True,'diagnostic_only':True,'bias_xy_ut':neo['bias'],'matrix_xy':neo['matrix'],'yaw_sign':neo['yaw_sign'],
      ...
      'neo3_ap_periph_residual_candidate':{'valid':neo_residual['valid'],'stage1_only':True,'owner':'ap_periph','host_hard_soft_iron':False,
        'yaw_sign':neo_residual['yaw_sign'],'yaw_offset_rad':neo_residual['yaw_offset_rad'],'heading_lut_input_rad':neo_residual['heading_lut_input_rad'],
        'heading_lut_correction_rad':neo_residual['heading_lut_correction_rad'],'validation':neo_residual['validation'],
        'field_norm_p05_ut':neo_residual['field_norm_p05_ut'],'field_norm_p95_ut':neo_residual['field_norm_p95_ut']}}
