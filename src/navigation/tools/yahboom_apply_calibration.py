...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--workspace',default=os.environ.get('AGV_ROOT', str(Path.home()/'agv')));ap.add_argument('--calibration',default='');ap.add_argument('--state',default='');a=ap.parse_args();ws=Path(a.workspace).expanduser().resolve();cal=Path(a.calibration) if a.calibration else ws/'calibration/yahboom_mag_planar_latest.yaml';state=Path(a.state) if a.state else ws/'calibration/yahboom_calibration_state.json'
    ...
    mag=ws/'src/navigation/config/mag_heading.yaml';evidence=ws/'src/navigation/config/yahboom_mag_calibration.yaml'
    ...
    out={'ok':True,'message':'Yahboom calibration applied atomically; mag_heading_fusion restart required','mag_heading_yaml':str(mag),'evidence_yaml':str(evidence),'direction':q.get('direction'),'rms_error_deg':v.get('rms_error_deg'),'max_abs_error_deg':v.get('max_abs_error_deg')};print(json.dumps(out));return 0
