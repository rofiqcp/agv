...
class Logger(Node):
    def __init__(self):
        ...
        self.w.writerow(['wall_time','ros_ns','state','segment','imu_yaw_deg','inertial_yaw_deg','validated_yaw_deg','validated_ok','neo_yaw_deg','gnss_cog_yaw_deg','gyro_x_rps','gyro_y_rps','gyro_z_rps','gyro_norm_rps','acc_x','acc_y','acc_z','acc_norm','yah_mag_x_lsb','yah_mag_y_lsb','yah_mag_z_lsb','yah_mag_norm_lsb','neo_mag_x_ut','neo_mag_y_ut','neo_mag_z_ut','neo_mag_norm_ut'])
    ...
    def on_imu(self,m):
        ...
        inert=self.fresh('inertial'); val=self.fresh('validated_yaw'); neo=self.fresh('neo_yaw'); cog=self.fresh('gnss_cog'); ym=self.fresh('yahmag',default=(float('nan'),)*4); nm=self.fresh('neomag',default=(float('nan'),)*4)
        ...
        row=[time.time(),ros_ns,self.state,seg,qdeg(imu),qdeg(inert),qdeg(val),int(self.validated),qdeg(neo),qdeg(cog),gx,gy,gz,gn,ax,ay,az,an,*ym,*nm]
        self.w.writerow(row)
    ...
    def write_summary(self):
        cols=['segment','samples','duration_sec','imu_mean_deg','imu_std_deg','inertial_mean_deg','inertial_std_deg','validated_mean_deg','validated_std_deg','neo_mean_deg','neo_std_deg','inertial_neo_residual_deg','gyro_z_mean_rps','gyro_norm_mean_rps','acc_norm_mean','yah_mag_norm_mean_lsb','neo_mag_norm_mean_ut']
