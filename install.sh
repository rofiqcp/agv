...
if [[ "$(basename "$ROOT")" != "agv" ]]; then
  echo "[WARN] nama folder project bukan 'agv': $ROOT" >&2
fi
...
/usr/bin/python3 "$ROOT/src/navigation/test/project_consistency_self_check.py"
/usr/bin/python3 "$ROOT/src/navigation/test/web_gui_self_check.py"
