...
function renderExperimentTable(){
  ...
  box.innerHTML=all.map((cols,ti)=>{const title=x.tableNames?.[ti]||`Tabel ${ti+1}`,head=timed?['t [s]',...cols]:cols,rows=tableRunRows.get(ti)||[];let body;if(rows.length)body=rows.slice(-300).map(r=>'<tr>'+((timed?[String(r.sec),...r.values]:r.values).map(v=>`<td>${escapeHtml(v)}</td>`).join(''))+'</tr>').join('');else{const vals=cols.map(c=>inferColumnValue(c));body='<tr class="live-preview-row">'+((timed?['0',...vals]:vals).map(v=>`<td>${escapeHtml(v)}</td>`).join(''))+'</tr>'}const cross=window.reportTableCrossButtons?.(ti)||'';return `<section class="direct-table-card"><div class="direct-table-title"><span>Tabel ${ti+1}</span><b>${escapeHtml(title)}</b><small>${rows.length?(recordStartedMs?'RECORDING':'RECORDED')+' • '+rows.length+' row':'LIVE PREVIEW'}</small>${cross}</div><div class="table-scroll"><table class="data-table experiment-data-table"><thead><tr>${head.map(c=>`<th>${escapeHtml(c)}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table></div>${window.reportLinkedGraphHtml?.(ti,title)||''}</section>`}).join('');
