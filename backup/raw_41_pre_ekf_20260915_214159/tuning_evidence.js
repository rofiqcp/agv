...
function renderExperimentTable(){
  ...
  box.innerHTML=all.map((cols,ti)=>{
    const title=x.tableNames?.[ti]||`Tabel ${ti+1}`,head=timed?['t [s]',...cols]:cols,rows=tableRunRows.get(ti)||[];
    let body='';
    if(rows.length)body=rows.slice(-300).map(r=>'<tr>'+((timed?[String(r.sec),...r.values]:r.values).map(v=>`<td>${escapeHtml(v)}</td>`).join(''))+'</tr>').join('');
    ...
    else{const vals=cols.map(c=>inferColumnValue(c));body='<tr class="live-preview-row">'+((timed?['0',...vals]:vals).map(v=>`<td>${escapeHtml(v)}</td>`).join(''))+'</tr>'}
...
async function stopWebRecording(){
  try{
    ...
    await stopAutomaticTrialMotion();captureLabTelemetry();if(currentExp==='navigation'&&selectedExperiment?.reportMode===true){reportEvidenceFrozen=true;reportEvidenceFrozenLeaf=selectedExperiment.id}drawAllExperimentCharts();window.reportRefreshGraphSourceTables?.(true);const savedTables=await saveAllTemplateTablesServer();
